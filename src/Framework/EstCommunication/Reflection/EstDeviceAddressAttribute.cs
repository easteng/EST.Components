...
// Type: EstCommunication.Reflection.EstDeviceAddressAttribute
...
{
  /// <summary>
  /// 应用于Est组件库读取的动态地址解析，具体用法为创建一个类，创建数据属性，如果这个属性需要绑定PLC的真实数据，就在属性的特性上应用本特性。<br />
  /// Applied to the dynamic address resolution read by the Est component library, the specific usage is to create a class and create data attributes.
  ...
  public class EstDeviceAddressAttribute : Attribute
  {
    /// <summary>
    ...
    public EstDeviceAddressAttribute(string address)
    {
    ...
    public EstDeviceAddressAttribute(string address, Type deviceType)
    {
      this.Address = address;
      this.Length = -1;
    ...
    public EstDeviceAddressAttribute(string address, int length)
    {
      this.Address = address;
      this.Length = length;
      this.DeviceType = (Type) null;
    ...
    public EstDeviceAddressAttribute(string address, int length, Type deviceType)
    {
      this.Address = address;
      this.Length = length;
      this.DeviceType = deviceType;
    ...
    /// <inheritdoc />
    public override string ToString() => string.Format("EstDeviceAddressAttribute[{0}:{1}]", (object) this.Address, (object) this.Length);
  }
}
