...
// Type: EstCommunication.NamespaceDoc
...
namespace EstCommunication
{
  ...
  /// <br /><br />
  /// <note type="important">
  /// 本组件的目标是集成一个框架，统一所有的设备读写方法，抽象成统一的接口<see cref="T:EstCommunication.Core.IReadWriteNet" />，对于上层操作只需要关注地址，读取类型即可，另一个目标是使用本框架轻松实现C#后台+C#客户端+web浏览器+android手机的全方位功能实现。
  /// </note>
  ...
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\Active.cs" region="Sample1" title="激活示例" />
  /// 在你的应用程序刚开起来的时候，激活一次即可，后续都不需要再重复激活了。接下来就可以开始写代码了。任何的设备的操作基本是相同的，实例化，配置参数（有些plc默认的参数即可），连接设备，读写操作，关闭
  /// <br />
  /// 关于Est的日志功能，贯穿整个hslcommunication的项目，所有的网络类，都包含了<see cref="T:EstCommunication.LogNet.ILogNet" />日志功能，当然你也可以继承接口实现你自己的日志，在hsl里提供了三种常见的简单实用的日志
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\LogNet\LogNetSingle.cs" region="Example1" title="单文件实例化" />
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\LogNet\LogNetSingle.cs" region="Example4" title="日志基本的使用" />
  /// <br />
  ...
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\Profinet\melsecTest.cs" region="Check Netstatus" title="检查连接状态" />
  /// 实际这种操作是非常不可取的，为什么这么说，下面说说原因：<br />
  /// <note type="warning">
  /// 首先说明下<see cref="M:EstCommunication.Core.Net.NetworkDoubleBase.ConnectServer" />方法里发生了什么？这个方法首先会关闭连接，然后重新连接，连接成功，就发送初始化指令（有些PLC就需要握手确认），初始化握手成功，才返回真正的成功！<br />
  /// 那么这里为什么不行呢？因为Read和Write方法是有混合锁实现互斥操作的，这样的好处就是多线程调用互不影响，但是<see cref="M:EstCommunication.Core.Net.NetworkDoubleBase.ConnectServer" />方法，并没有互斥锁，如果调用的时候同时在读写，那就会导致异常，
  /// 那么为什么没有加互斥锁呢？因为为了实现读写方法的时候，支持自动重连操作，所以连接方法已经在互斥锁了。如果再加互斥锁，会发生死锁，所以综合考虑，就设计成了现在的样子。
  /// </note>
  /// 既然上面的代码不能使用，那么怎么来看当前的连接状态呢？这里有一点需要注意，<see cref="M:EstCommunication.Core.Net.NetworkDoubleBase.ConnectServer" />只需要调用0次或1次即可。
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\Profinet\melsecTest.cs" region="Check Netstatus2" title="检查连接状态" />
  /// 如果你本来就在每秒读取PLC的数据信息了，那么连检测的定时器都不用写了，你每次读取数据的时候顺便判定下，结果就出来了。<br /><br />
  /// 其他相关的代码示例需要到各自的目录里查找，下面只列举了一些常见的代码示例
  /// <list type="bullet">
  ///     <item>Est组件日志相关示例参考<see cref="T:EstCommunication.LogNet.ILogNet" /></item>
  ///     <item>三菱mc协议示例参考<see cref="T:EstCommunication.Profinet.Melsec.MelsecMcNet" /></item>
  ///     <item>西门子S7协议示例参考<see cref="T:EstCommunication.Profinet.Siemens.SiemensS7Net" /></item>
  ///     <item>欧姆龙协议示例参考<see cref="T:EstCommunication.Profinet.Omron.OmronFinsNet" /></item>
  ///     <item>罗克韦尔协议示例参考<see cref="T:EstCommunication.Profinet.AllenBradley.AllenBradleyNet" /></item>
  ///     <item>Modbus协议示例参考<see cref="T:EstCommunication.ModBus.ModbusTcpNet" /></item>
  ///     <item>MQTT服务器示例参考<see cref="T:EstCommunication.MQTT.MqttServer" /></item>
  ///     <item>MQTT客户端示例参考<see cref="T:EstCommunication.MQTT.MqttClient" /></item>
  ///     <item>WebSocket服务器示例参考<see cref="T:EstCommunication.WebSocket.WebSocketServer" /></item>
  ///     <item>WebSocket客户端示例参考<see cref="T:EstCommunication.WebSocket.WebSocketClient" />。</item>
  ///     <item>WebApi示例参考<see cref="T:EstCommunication.Enthernet.HttpServer" />。</item>
  ///     <item>Redis示例参考<see cref="T:EstCommunication.Enthernet.Redis.RedisClient" /></item>
  ///     <item>身份证阅读器串口版<see cref="T:EstCommunication.Profinet.IDCard.SAMSerial" /></item>
  ///     <item>身份证阅读器网口版<see cref="T:EstCommunication.Profinet.IDCard.SAMTcpNet" /></item>
  ///     <item>文件传送服务器<see cref="T:EstCommunication.Enthernet.UltimateFileServer" /></item>
  /// </list>
  /// <note type="important">
  ...
  /// <br />
  /// <list type="bullet">
  ///     <item>本群提供专业版通讯库的所有更新版的 EstCommunication 源代码。包含 .Net Java Python 三大平台。</item>
  ...
  /// 
  /// 付费二维码：<br />
  /// <img src="https://raw.githubusercontent.com/dathlin/EstCommunication/master/imgs/support.png" />
  /// </remarks>
  /// <revisionHistory>
  ...
  ///             <item>将部分类从EstCommunication命名空间下移动到EstCommunication.Core下面。</item>
  ///             <item>提供了一个通用的ModBus TCP的客户端类，方便和服务器交互。</item>
  ///             <item>完善了EstCommunication.BasicFramework.SoftBaisc下面的辅助用的静态方法，提供了一些方便的数据转化，在上面进行公开。</item>
  ///         </list>
  ///     </revision>
  ...
  ///             <item>VIP群将免费使用全新的控件库，谢谢支持。地址：https://github.com/dathlin/EstControlsDemo </item>
  ///         </list>
  ///     </revision>
  ...
  ///             <item>新增EstDeviceAddress特性类，现在支持直接基于对象的读写操作，提供了一种更加便捷的读写数据的机制，详细的关注后续的论坛。</item>
  ...
  ///             <item>发布基于HSL扩展组件EstCppExtension，将写入的重载方法名按照类型重写一遍，方便C++调用。</item>
  ///             <item>VC++的demo示例，新增写入数据的例子，基于扩展组件EstCppExtension实现，详细参照demo源代码。</item>
  ...
  ///         <list type="bullet">
  ///             <item>EstExtension: 完善一些转化的api，方便数组和字符串转化，完善对象转JSON字符串。</item>
  ...
  ///             <item>Python：实现了python版本的EstCommunication程序，基于pyqt实现，初步添加了一些PLC的调试界面。</item>
  ///             <item>代码注释优化，使用前请仔细阅读下面的信息：http://api.hslcommunication.cn </item>
  ///             <item>http://www.hslcommunication.cn/MesDemo 官网的地址以后作为优秀的MES产品展示平台，欢迎大家关注。</item>
  ///             <item>HSL的目标是打造成工业互联网的利器，工业大数据的基础，打造边缘计算平台。</item>
  ///         </list>
  ...
  ///             <item>UdpNet：添加ConnectionId属性，使用的<seealso cref="M:EstCommunication.BasicFramework.SoftBasic.GetUniqueStringByGuidAndRandom" />方法获取信息。</item>
  ...
  ///             <item>EstExtension: 增加IncreaseBy方法，但是测试发现不适用byte类型。</item>
  ///             <item>Omron-cip: 读写字符串仍然没有测试通过，请暂时不要调用。</item>
  ///             <item>官网的备案失效了，重新备案需要点时间，请访问 http://118.24.36.220 然后去顶部的菜单找相应的入口。</item>
  ///             <item>本软件已经申请软件著作权，软著登字第5219522号，任何盗用软件，破解软件，未经正式合同授权而商业使用均视为侵权。</item>
  ///             <item>HSL的目标是打造成工业互联网的利器，工业大数据的基础，打造边缘计算平台。</item>
  ...
  ///             <item>Demo: EstCommunication Test Demo支持PLC及一些连接对象的参数保存功能，使用英文冒号可以分类管理。</item>
  ...
  ///             <item>PLC: 所以的PLC实现了EstMqttApi特性支持，从而在MqttServer里可以直接注册，然后对外开放读写接口操作。</item>
  ///             <item>官网地址： http://www.hslcommunication.cn 官网的界面全新设计过，感谢浏览关注。</item>
  ///             <item>本软件已经申请软件著作权，软著登字第5219522号，任何盗用软件，破解软件，未经正式合同授权而商业使用均视为侵权。</item>
  ///         </list>
  ///     </revision>
  ...
  ///             <item>EstCommunication: 所有的异步通信代码优化，优化超时检测机制，现在大大提升了服务器的高并发的能力，异步通信的性能。</item>
  ...
  ///             <item>EstExtension: 新增从字节数组获取位值的扩展方法: GetBoolValue( this byte[] bytes, int bytIndex, int boolIndex )</item>
