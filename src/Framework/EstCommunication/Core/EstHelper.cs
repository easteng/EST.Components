...
// Type: EstCommunication.Core.EstHelper
...
{
  /// <summary>
  /// EstCommunication的一些静态辅助方法<br />
  /// Some static auxiliary methods of EstCommunication
  /// </summary>
  public class EstHelper
  {
    ...
    /// <summary>
    /// 解析地址的附加<see cref="T:EstCommunication.Core.DataFormat" />参数方法，比如你的地址是format=ABCD;D100，可以提取出"format"的值的同时，修改地址本身，如果"format"不存在的话，返回默认的<see cref="T:EstCommunication.Core.IByteTransform" />对象<br />
    /// Parse the additional <see cref="T:EstCommunication.Core.DataFormat" /> parameter method of the address. For example, if your address is format=ABCD;D100,
    /// you can extract the value of "format" and modify the address itself. If "format" does not exist,
    /// Return the default <see cref="T:EstCommunication.Core.IByteTransform" /> object
    /// </summary>
    /// <param name="address">复杂的地址格式，比如：s=100;D100</param>
