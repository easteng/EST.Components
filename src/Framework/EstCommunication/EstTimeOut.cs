...
// Type: EstCommunication.EstTimeOut
...
namespace EstCommunication
{
  ...
  public class EstTimeOut
  {
    ...
    /// <summary>
    /// 操作是否成功，当操作完成的时候，需要设置为<c>True</c>，超时检测自动结束。如果一直为<c>False</c>，超时检测到超时，设置<see cref="P:EstCommunication.EstTimeOut.IsTimeout" />为<c>True</c><br />
    ...
    /// the timeout is detected by the timeout, set <see cref="P:EstCommunication.EstTimeOut.IsTimeout" /> to <c>True</c>
    /// </summary>
    ...
    /// <summary>
    /// 是否发生了超时的操作，当调用方因为异常结束的时候，需要对<see cref="P:EstCommunication.EstTimeOut.IsTimeout" />进行判断，是否因为发送了超时导致的异常<br />
    /// Whether a timeout operation has occurred, when the caller ends abnormally,
    /// it needs to judge <see cref="P:EstCommunication.EstTimeOut.IsTimeout" />, whether it is an exception caused by a timeout sent
    /// </summary>
    ...
    /// <summary>
    /// 新增一个超时检测的对象，当操作完成的时候，需要自行标记<see cref="T:EstCommunication.EstTimeOut" />对象的<see cref="P:EstCommunication.EstTimeOut.IsSuccessful" />为<c>True</c><br />
    /// Add a new object for timeout detection. When the operation is completed,
    /// you need to mark the <see cref="P:EstCommunication.EstTimeOut.IsSuccessful" /> of the <see cref="T:EstCommunication.EstTimeOut" /> object as <c>True</c>
    /// </summary>
    ...
    /// <summary>
    /// 新增一个超时检测的对象，需要指定socket，超时时间，返回<see cref="T:EstCommunication.EstTimeOut" />对象，用作标记完成信息<br />
    /// Add a new object for timeout detection, you need to specify the socket, the timeout period,
    /// and return the <see cref="T:EstCommunication.EstTimeOut" /> object for marking completion information
    /// </summary>
    ...
    /// <summary>
    /// 整个EstCommunication的检测超时的核心方法，由一个单独的线程运行，线程的优先级很高，当前其他所有的超时信息都可以放到这里处理<br />
    /// The core method of detecting the timeout of th e entire EstCommunication is run by a separate thread.
