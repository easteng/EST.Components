...
// Type: EstCommunication.Enthernet.Redis.RedisSubscribe
...
namespace EstCommunication.Enthernet.Redis
{
  ...
  public class RedisSubscribe : NetworkXBase
  {
    ...
    /// <summary>
    /// 如果Redis服务器设置了密码，此处就需要进行设置。必须在 <see cref="M:EstCommunication.Enthernet.Redis.RedisSubscribe.ConnectServer" /> 方法调用前设置。<br />
    /// If the Redis server has set a password, it needs to be set here. Must be set before the <see cref="M:EstCommunication.Enthernet.Redis.RedisSubscribe.ConnectServer" /> method is called.
    /// </summary>
    public string Password { get; set; }

    /// <summary>
    /// 获取或设置当前连接超时时间，主要对 <see cref="M:EstCommunication.Enthernet.Redis.RedisSubscribe.ConnectServer" /> 方法有影响，默认值为 5000，也即是5秒。<br />
    /// Get or set the current connection timeout period, which mainly affects the <see cref="M:EstCommunication.Enthernet.Redis.RedisSubscribe.ConnectServer" /> method. The default value is 5000, which is 5 seconds.
    /// </summary>
    ...
    /// <inheritdoc cref="M:EstCommunication.Enthernet.Redis.RedisSubscribe.SubscribeMessage(System.String)" />
