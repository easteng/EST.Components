...
// Type: EstCommunication.Enthernet.UltimateFileServer
...
namespace EstCommunication.Enthernet
{
  ...
  /// 客户端使用<see cref="T:EstCommunication.Enthernet.IntegrationFileClient" />进行访问，支持上传，下载，删除，请求文件列表，校验文件是否存在操作。<br />
  ...
  /// supports files with the same name, and the client uses <see cref="T:EstCommunication.Enthernet.IntegrationFileClient" /> to access,
