...
// Type: EstCommunication.LogNet.LogNetSingle
...
namespace EstCommunication.LogNet
{
  ...
  public class LogNetSingle : LogNetBase, ILogNet, IDisposable
  {
    ...
    /// <summary>
    /// 实例化一个单文件日志的对象，如果日志的路径为空，那么就不存储数据，只触发<see cref="E:EstCommunication.LogNet.LogNetBase.BeforeSaveToFile" />事件<br />
    /// Instantiate a single file log object. If the log path is empty, then no data is stored and only the <see cref="E:EstCommunication.LogNet.LogNetBase.BeforeSaveToFile" /> event is triggered.
    /// </summary>
