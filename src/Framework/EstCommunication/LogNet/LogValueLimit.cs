...
// Type: EstCommunication.LogNet.LogValueLimit
...
namespace EstCommunication.LogNet
{
  ...
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\LogNet\LogValueLimitSample.cs" region="Sample1" title="简单的记录调用次数" />
  /// 因为这个数据是保存在内存里的，程序重新运行就丢失了，如果希望让这个数据一直在程序的话，在软件退出的时候需要存储文件，在软件启动的时候，加载文件数据
  /// <code lang="cs" source="EstCommunication_Net45.Test\Documentation\Samples\LogNet\LogValueLimitSample.cs" region="Sample2" title="存储与加载" />
  /// </example>
  public class LogValueLimit : LogStatisticsBase<ValueLimit>
  {
    ...
    /// <inheritdoc cref="M:EstCommunication.LogNet.LogStatisticsBase`1.#ctor(EstCommunication.LogNet.GenerateMode,System.Int32)" />
    ...
    /// <summary>
    /// 从指定的文件加载对应的统计信息，通常是调用<see cref="M:EstCommunication.LogNet.LogValueLimit.SaveToFile(System.String)" />方法存储的文件，如果文件不存在，将会跳过加载<br />
    /// Load the corresponding statistical information from the specified file, usually the file stored by calling the <see cref="M:EstCommunication.LogNet.LogValueLimit.SaveToFile(System.String)" /> method.
