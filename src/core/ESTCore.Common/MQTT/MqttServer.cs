...
// Type: ESTCore.Common.MQTT.MqttServer
...
namespace ESTCore.Common.MQTT
{
    ...
    /// <example>
    /// 最简单的使用，就是实例化，启动服务即可
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample1" title="简单的实例化" />
    /// 当然了，我们可以稍微的复杂一点，加一个功能，验证连接的客户端操作
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample2" title="增加验证" />
    /// 我们可以对ClientID，用户名，密码进行验证，那么我们可以动态修改client id么？比如用户名密码验证成功后，client ID我想设置为权限等级。
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample2_1" title="动态修改Client ID" />
    /// 如果我想强制该客户端不能主动发布主题，可以这么操作。
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample2_2" title="禁止发布主题" />
    /// 你也可以对clientid进行过滤验证，只要结果返回不是0，就可以了。接下来我们实现一个功能，所有客户端的发布的消息在控制台打印出来,
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample3" title="打印所有发布" />
    /// 捕获客户端刚刚上线的时候，方便我们进行一些额外的操作信息。下面的意思就是返回一个数据，将数据发送到指定的会话内容上去
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample4" title="客户端上线信息" />
    /// 下面演示如何从服务器端发布数据信息，包括多种发布的方法，消息是否驻留，详细看说明即可
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample5" title="服务器发布" />
    /// 下面演示如何支持同步网络访问，当客户端是同步网络访问时，协议内容会变成HUSL，即被视为同步客户端，进行相关的操作，主要进行远程调用RPC，以及查询MQTT的主题列表。
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample6" title="同步访问支持" />
    /// 如果需要查看在线信息，可以随时获取<see cref="P:ESTCore.Common.MQTT.MqttServer.OnlineCount" />属性，如果需要查看报文信息，可以实例化日志，参考日志的说明即可。<br /><br />
    ...
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample7" title="基于MQTT的RPC接口实现" />
    /// 如果需要查看在线信息，可以随时获取<see cref="P:ESTCore.Common.MQTT.MqttServer.OnlineCount" />属性，如果需要查看报文信息，可以实例化日志，参考日志的说明即可。<br /><br />
    /// 最后介绍一下文件管理服务是如何启动的，在启动了文件管理服务之后，其匹配的客户端 <see cref="T:ESTCore.Common.MQTT.MqttSyncClient" /> 就可以上传下载，遍历文件了。
    /// 而服务器端做的就是启用服务，如果你需要一些更加自由的权限控制，比如某个账户只能下载，不能其他操作，都是可以实现的。更加多的示例参考DEMO程序。
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\MQTT\MqttServerSample.cs" region="Sample8" title="基于MQTT的文件管理服务启动" />
    /// </example>
    public class MqttServer : NetworkServerBase
    {
        ...
        /// <inheritdoc cref="M:ESTCore.Common.MQTT.MqttServer.RegisterMqttRpcApi(System.String,System.Object)" />
        ...
        /// <summary>
        /// 当客户端进行文件操作时，校验客户端合法性的事件，操作码具体查看<seealso cref="T:ESTCore.Common.MQTT.MqttControlMessage" />的常量值<br />
        /// When client performing file operations, it is an event to verify the legitimacy of the client. For the operation code, check the constant value of <seealso cref="T:ESTCore.Common.MQTT.MqttControlMessage" />
        /// </summary>
        ...
        private void DeleteExsistingFile(string path, string fileName) => this.DeleteExsistingFile(path, new List<string>()
    {
      fileName
        ...
        /// <summary>
        ///  当收到客户端发来的<see cref="T:ESTCore.Common.MQTT.MqttClientApplicationMessage" />消息时触发<br />
        ///  Triggered when a <see cref="T:ESTCore.Common.MQTT.MqttClientApplicationMessage" /> message is received from the client
        /// </summary>
        ...
        /// <inheritdoc cref="P:ESTCore.Common.Enthernet.HttpServer.LogStatistics" />
        ...
        /// <summary>
        /// 当客户端进行文件操作时，校验客户端合法性的委托，操作码具体查看<seealso cref="T:ESTCore.Common.MQTT.MqttControlMessage" />的常量值<br />
        /// When client performing file operations, verify the legitimacy of the client, and check the constant value of <seealso cref="T:ESTCore.Common.MQTT.MqttControlMessage" /> for the operation code.
        /// </summary>
