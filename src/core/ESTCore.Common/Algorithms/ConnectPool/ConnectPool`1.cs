...
// Type: ESTCore.Common.Algorithms.ConnectPool.ConnectPool`1
...
namespace ESTCore.Common.Algorithms.ConnectPool
{
    ...
    /// <remarks>
    /// 需要先实现 <see cref="T:ESTCore.Common.Algorithms.ConnectPool.IConnector" /> 接口的对象，然后就可以实现真正的连接池了，理论上可以实现任意的连接对象，包括modbus连接对象，各种PLC连接对象，数据库连接对象，redis连接对象，SimplifyNet连接对象等等。下面的示例就是modbus-tcp的实现
    /// <note type="warning">要想真正的支持连接池访问，还需要服务器支持一个端口的多连接操作，三菱PLC的端口就不支持，如果要测试示例代码的连接池对象，需要使用本组件的<see cref="T:ESTCore.Common.ModBus.ModbusTcpServer" />来创建服务器对象</note>
    /// </remarks>
    /// <example>
    ...
    /// 然后就可以实现真正的连接池了
    /// <code lang="cs" source="ESTCore.Common_Net45.Test\Documentation\Samples\Algorithms\ConnectPool.cs" region="ConnectPoolExample" title="ConnectPool示例" />
    /// </example>
    public class ConnectPool<TConnector> where TConnector : IConnector
    {
        ...
        /// <summary>
        /// 获取一个可用的连接对象，如果已经达到上限，就进行阻塞等待。当使用完连接对象的时候，需要调用<see cref="M:ESTCore.Common.Algorithms.ConnectPool.ConnectPool`1.ReturnConnector(`0)" />方法归还连接对象。<br />
        ...
        /// you need to call the <see cref="M:ESTCore.Common.Algorithms.ConnectPool.ConnectPool`1.ReturnConnector(`0)" /> method to return the connection object.
        /// </summary>
